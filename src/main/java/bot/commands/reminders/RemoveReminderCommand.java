...
public class RemoveReminderCommand implements Command {
    ...
    public void execute(TelegramBot bot, Message message, String[] args) {
        ...
        try {
            ...
            StringBuilder sb = new StringBuilder("Какое напоминание удалить?\n");
            for (int i = 0; i < names.size(); i++) {
                sb.append(i + 1).append(". ").append(names.get(i)).append("\n");
            }
            bot.sendMessage(chatId, sb.toString());
            bot.sendMessage(chatId, "Введите имя напоминания для удаления:");
            fsm.setState(userId, UserState.AWAITING_REMINDER_NAME_TO_REMOVE);
            fsm.setTempData(userId, "command", "removeReminder");
        } catch (Exception e) {
