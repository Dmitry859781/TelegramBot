...
class ShowNoteCommandTest extends AbstractBotTest {
    ...
    void testExecuteSendsNoteListAndSetsState() throws SQLException {
        ...
        try (MockedStatic<BotFSM> mockedFsm = Mockito.mockStatic(BotFSM.class);
            ...
            ShowNoteCommand command = new ShowNoteCommand();

            // When
            command.execute(bot, message, new String[]{});

            // Then
            verify(bot).sendMessage(chatId, "Какую заметку хотите посмотреть?\n1. note1\n2. note2");
            verify(bot).sendMessage(chatId, "Введите имя заметки, которую хотите посмотреть.");
            verify(fsm).setState(userId, UserState.AWAITING_NOTE_NAME_TO_SHOW);
        }
    }
    ...
    void testExecuteHandlesException() throws SQLException {
        when(noteService.getUserNotes(userId)).thenThrow(new SQLException("Database error"));
    ...
    void testGetters() {
        ...
        assertEquals("showNote", command.getCommandName());
        assertEquals("Показать заметку", command.getDescription());
        assertEquals("/showNote", command.getUsage());
    }
}