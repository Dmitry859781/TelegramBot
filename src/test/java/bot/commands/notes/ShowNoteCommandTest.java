...
class ShowNoteCommandTest {
    ...
    private ShowNoteCommand showNoteCommand;

    @BeforeEach
    void setUp() {
        showNoteCommand = new ShowNoteCommand(noteService, fsm);
    }
    ...
    void testExecuteSendsNoteListAndSetsState() throws SQLException {
        ...
        // When
        showNoteCommand.execute(bot, message, new String[]{});

        // Then
        verify(bot).sendMessage(eq(chatId), eq("Какую заметку хотите посмотреть?\n1. note1\n2. note2"));
        verify(bot).sendMessage(eq(chatId), eq("Введите имя заметки, которую хотите посмотреть."));
        verify(fsm).setState(eq(userId), eq(UserState.AWAITING_NOTE_NAME_SHOW));
    }
