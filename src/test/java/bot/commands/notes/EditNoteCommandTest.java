...
class EditNoteCommandTest {
    ...
    @BeforeEach
    void setUp() {
        editNoteCommand = new EditNoteCommand(noteService, fsm);
    }
    ...
    void testExecuteSendsNoteListAndSetsState() throws SQLException {
        ...
        // When
        editNoteCommand.execute(bot, message, new String[]{});

        // Then
        verify(bot).sendMessage(eq(chatId), eq("Какую заметку хотите отредактировать?\n1. note1\n2. note2"));
        verify(bot).sendMessage(eq(chatId), eq("Введите имя редактируемой заметки."));
        verify(fsm).setState(eq(userId), eq(UserState.AWAITING_NOTE_NAME_EDIT));
    }
