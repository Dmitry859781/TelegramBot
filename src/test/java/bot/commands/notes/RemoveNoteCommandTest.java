...
class RemoveNoteCommandTest extends AbstractBotTest {
    ...
    void testExecuteSendsNoteListAndSetsState() throws SQLException {
        ...
        try (MockedStatic<BotFSM> mockedFsm = Mockito.mockStatic(BotFSM.class);
            ...
            // Then
            verify(bot).sendMessage(chatId, "Какую заметку хотите удалить?\n1. note1\n2. note2");
            verify(bot).sendMessage(chatId, "Введите имя заметки, которую хотите удалить.");
            verify(fsm).setState(userId, UserState.AWAITING_NOTE_NAME_TO_REMOVE);
        }
    }
    ...
    void testGetters() {
        ...
        assertEquals("removeNote", command.getCommandName());
        assertEquals("Удалить заметку", command.getDescription());
        assertEquals("/removeNote", command.getUsage());
    }
}