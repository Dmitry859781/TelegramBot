...
class HandleAwaitingNoteNewNameTest extends AbstractBotTest {
    ...
    @Test
    @DisplayName("Тест handle: успешный поиск заметки и переход к ожиданию нового имени")
    void testHandle_Success() throws SQLException {
        // Given заметка существует
        when(noteService.getNote(userId, "old_name")).thenReturn("какой-то текст заметки");
        ...
        try (MockedStatic<NoteService> mockedNoteService = Mockito.mockStatic(NoteService.class);
            ...
            HandleAwaitingNoteNewName handler = new HandleAwaitingNoteNewName();

            // When
            handler.handle(userId, chatId, "old_name", bot);

            // Then
            verify(noteService).getNote(userId, "old_name");
            verify(fsm).setTempData(userId, "oldNoteName", "old_name");
    ...
    void testHandle_NoteNotFound() throws SQLException {
        // Given заметка не существует (возвращает null)
